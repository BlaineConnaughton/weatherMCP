...
async def get_alerts(state: str) -> str:
    ...
    """
    # Basic input validation: expect two-letter state code
    if not isinstance(state, str) or len(state.strip()) != 2 or not state.strip().isalpha():
...
async def get_forecast(latitude: float, longitude: float) -> str:
    ...
    try:
        ...
        return "Invalid latitude/longitude. Provide numeric values for latitude and longitude."
